...
# Optional explicit assignment overrides (recommended for non-dev ops)
# Examples:
#   export ASSIGN_BY_TYPE_JSON='{"REVISAO":18,"TELEMETRIA_OFF":18,"ALERTA_TECNICO":166}'
#   export ASSIGN_BY_BRANCH_JSON='{"Sinop":150,"Varzea Grande":18}'
...
ASSIGN_BY_TYPE_JSON = _load_json_env("ASSIGN_BY_TYPE_JSON")
ASSIGN_BY_BRANCH_JSON = _load_json_env("ASSIGN_BY_BRANCH_JSON")
...
def choose_assignee(users:List[B24User], dept_id:Optional[int], serial:str, opp_type:str)->int:
    # 0) explicit override by type
    v = ASSIGN_BY_TYPE_JSON.get(opp_type)
...
def mark_status(conn, opp_id:int, status:str)->None:
    # Never mutate DB when running in dry-run mode
    if DRY_RUN:
        return
    ...
    with eng.begin() as conn:
        ...
        for row in opps:
            ...
            try:
                ...
                # 0) explicit override by branch (optional)
                bkey = branch.strip()
                if bkey and (bkey in ASSIGN_BY_BRANCH_JSON) and str(ASSIGN_BY_BRANCH_JSON[bkey]).isdigit():
                    assigned = int(ASSIGN_BY_BRANCH_JSON[bkey])
                else:
