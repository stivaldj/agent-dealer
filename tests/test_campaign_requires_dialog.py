...
class _Db:
    ...
    def active_machine_ownerships(self):
        return [{"client_id": "C1", "machine_id": "M1", "phone": "+551111", "state": "MT", "model": "CASE"}]
    ...
    def campaign_targets(self, *, campaign_id: str, statuses=("NEW", "PENDING", "RETRY")):
        ...
        return [{"id": 1, "phone": "+551111", "client_id": "C1", "machine_id": "M1"}]
...
class _Bitrix:
    ...
    def create_task(self, title: str, summary: str, context: dict, idempotency_key=None):
        _ = title, summary, context, idempotency_key
        self.task_count += 1
        return {"ok": True}
...
def test_campaign_requires_dialog() -> None:
    db = _Db()
    bitrix = _Bitrix()
    ...
    assert db.last_status == "NEEDS_MANUAL_START"
    assert bitrix.task_count == 1
